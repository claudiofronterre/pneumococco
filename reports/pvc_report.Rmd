...
# Age distribution of subjects (without adults)
cocco %>% 
  filter(typ_cat7 != "adult") %>%
  ggplot(aes(x = age_y * 12, col = pcv_vaccd, fill = pcv_vaccd)) +
  geom_histogram(position = "identity", alpha = .5) +
  scale_x_continuous(breaks = round(seq(0, 135, by = 12))) +
  scale_color_manual(values = rev(pal_lancet()(2)), 
                     name = "", label = c("Unvaccinated", "Vaccinated")) +
  scale_fill_manual(values = rev(pal_lancet()(2)), 
                    name =  "", label = c("Unvaccinated", "Vaccinated")) +
  labs(title = "Age distribution of children", x = "age (months)") +
  theme_bw() 

# Age distribution of subjects per survey (without adults)
cocco %>% 
  filter(typ_cat7 != "adult") %>%
ggplot(aes(x = age_y * 12, col = pcv_vaccd, fill = pcv_vaccd)) +
  geom_histogram(position = "identity", alpha = .5, show.legend = F) +
  ...
  scale_fill_manual(values = rev(pal_lancet()(2)), 
                    name =  "", label = c("Unvaccinated", "Vaccinated")) +
  labs(title = "Age distribution of children per survey", x = "age (months)") +
  theme_bw() +
  theme(axis.text.x = element_text(size = 6))
```

```{r vt_dist_surv, out.height="48%", fig.align='left'}
# Ditribution of pneumococco carriage over different groups and surveys
ggplot(cocco, aes(x = pcv_vaccd, fill = factor(vt_01))) +
...
<!--   tcv <- tcv + .5 -->
<!--   tcu <- tcu + .5 -->
<!--   temp <- round(rbind(logistic(fit$results[1, ]), fit$results[4, ]), 2) -->
<!--   temp <- apply(temp, 1, function(x) paste0(x[1], " (", x[2], ", ", x[3], ")")) -->
<!--   temp <- cbind(age = tcv, alpha = temp[1], beta = temp[2]) -->
<!--   res <- rbind(res, temp) -->
<!-- } -->
<!-- ``` -->
