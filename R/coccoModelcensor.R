coccoModelcensor <- function(response,
                             vacc,
                             age,
                             data, 
                             ini.par, 
  ...
  par.start <- c(ini.par[1], ini.par[2], log(ini.par[3:length(ini.par)])) 
  ...
  if(delta2) {
    log.lik <- function(par) {
      ...
      pr[vacc] <- logistic(alpha + beta * age[vacc]) * (age[vacc] < kv) + 
        logistic(alpha + beta * kv) * exp(- deltav * (age[vacc] - kv)) * (age[vacc] >= kv)
      
      # Probability functional form (vaccinated)
      pr[unvacc] <- logistic(alpha + beta * age[unvacc]) * (age[unvacc] < ku) + 
        logistic(alpha + beta * ku) * exp(- deltau * (age[unvacc] - ku)) * (age[unvacc] >= ku)
  ...
  par <- c(estim2$par[1], estim2$par[2], exp(estim2$par[3:length(ini.par)]))
  if (delta2) {
    names(par) <- c("alpha", "beta", "deltau", "deltav")
  } else {
  ...
  results <- cbind(par, rbind(conf.int[1:2, ], exp(conf.int[3:length(ini.par), ])))
  ...
  return(list(results = results, convcode = estim2$convergence, lik = estim2$value, 
              ...
              par = estim2$par, covar = solve(estim2$hessian)))
}
