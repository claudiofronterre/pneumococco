censorModel <- function(formula = vt_01 ~ gender + crowd_index + ses_score + year,
    ...
    if(knot) {
      ...
      log.lik <- function(par) {
        ...
        pr[unvacc] <- alpha[unvacc] * beta * exp(- delta1 * (age[unvacc] - tcu))
        pr[vacc] <- alpha[vacc] * exp(- delta2 * (age[vacc] - tcu))
      ...
      conf.int <- cbind(estim2$par - z*sqrt(diag(solve(estim2$hessian))),
                        estim2$par + z*sqrt(diag(solve(estim2$hessian))))
      conf.int <- rbind(conf.int[1:p, ],
                        exp(conf.int[(p + 1):(p + 2),]),
                        conf.int[p + 3, ])
      results <- cbind(par, conf.int)
      colnames(results) <- c("Estimated", "Lower", "Upper")
      ...
      beta <-  as.numeric(par[p+3])
      predicted <- c(alpha[vacc] * exp(- delta2 * (age[vacc] - tcu)),
                     alpha[unvacc] * beta * exp(- delta1 * (age[unvacc] - tcu)))
      
      return(list(results = results, convcode = estim2$convergence, lik = estim2$value, 
                  AIC = 2 * length(fit$par) + 2 * estim2$value, y = y, 
                  predicted = predicted, id = id, par = estim2$par, covar = solve(estim2$hessian),
                  D = D, 
                  sd = sqrt(diag(solve(estim2$hessian)))))
    }
